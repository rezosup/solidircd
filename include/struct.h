...
#include <openssl/dh.h>/* OpenSSL stuff */
...
#define HOSTIPLEN	46	/* Length of ipv6 address */
#define IPLEN		16
...
#define FLAGS_URSL         0x000014   /* Flag for Unresolved clients */
#define	FLAGS_LISTEN       0x000020   /* used to mark clients which we listen()on */
#define FLAGS_HAVERECVQ	   0x000040   /* Client has full commands in their recvq */
#define	FLAGS_DOINGDNS	   0x000080   /* client is waiting for a DNS  response */
#define	FLAGS_AUTH	       0x000100   /* client is waiting on rfc931 response */
#define	FLAGS_WRAUTH	   0x000200   /* set if we havent writen to ident  server */
...
#define FLAGS_SOBSENT      0x100000   /* we've sent an SOB, just have to send an EOB */
#define FLAGS_EOBRECV      0x200000   /* we're waiting on an EOB */
#define FLAGS_BAD_DNS	   0x400000   /* spoofer-guy */
#define FLAGS_SERV_NEGO	   0x800000  /* This is a server that has passed  connection tests, but is a stat < 0 for handshake purposes */
...
#define FLAGS_MAP          0x10000000
#define FLAGS_SHUNNED      0x20000000 /* User is shunned */
#define FLAGS_GLINED       0x80000000 /* User is glined */
#ifdef  HAVE_SSL
#define FLAGS_SSL          0x40000000 /* SSL */
#endif
...
#define UMODE_y 0x00020000  /* umode +y - Notifys when users do Stats/links  & whois notices. */
...
#define UMODE_Y 0x02000000  /* umode +Y - Deaf Mode */
#define UMODE_C 0x04000000  /* umode +C - conops */
#define UMODE_W 0x08000000  /* umode +W - Hides Channels & Server for IRCop's Whois */
...
#ifdef DCCALLOW
#define ALL_UMODES  (UMODE_w|UMODE_a|UMODE_i|UMODE_v|UMODE_o|UMODE_r|\
                       UMODE_A|UMODE_Y|UMODE_R|UMODE_h|UMODE_s|UMODE_c|\
					   UMODE_r|UMODE_k|UMODE_K|UMODE_f|UMODE_y|UMODE_d|\
					   UMODE_g|UMODE_b|UMODE_n|UMODE_m|UMODE_O|UMODE_R|\
					   UMODE_v|UMODE_R|UMODE_C|UMODE_z|UMODE_H|UMODE_F|\
					    UMODE_j|UMODE_e|UMODE_W)
#else
...
#define USER_UMODES (UMODE_i|UMODE_v|UMODE_R|UMODE_Y)
...
#define OPER_UMODES (UMODE_a|UMODE_b|UMODE_c|UMODE_d|UMODE_e|UMODE_f|UMODE_g|\
                     ...
                     UMODE_K|UMODE_H|UMODE_C|UMODE_W|UMODE_s|UMODE_w)
#else
...
#define	CanBeOper(x)		((x)->umode & (UMODE_o|UMODE_O))
...
#define IsUmodeC(x)             ((x)->umode & UMODE_C)
#define IsUmodeH(x)             ((x)->umode & UMODE_H)
#define IsUmodeW(x)             ((x)->umode & UMODE_W)
...
#define SetUmodeH(x)            ((x)->umode |= UMODE_H)
#define SetUmodeW(x)            ((x)->umode |= UMODE_W)
...
#define CanUmodev(x)           ((x)->umode |= UMODE_v)
#ifdef DCCALLOW
...
#define IsURSL(x)               ((x)->flags & FLAGS_URSL)
#define SetURSL(x)              ((x)->flags |= FLAGS_URSL)
#define ClearURSL(x)            ((x)->flags &= ~FLAGS_URSL)
#endif

#ifdef HAVE_SSL
#define SetSSL(x)              ((x)->flags |= FLAGS_SSL)
#define ClearSSL(x)            ((x)->flags &= ~FLAGS_SSL)
#define IsSSL(x)               ((x)->flags & FLAGS_SSL)
#endif

#ifdef HAVE_SSL
#define CanUmodez(x)           (((x)->flags & FLAGS_SSL) && ((x)->ssl != NULL))
#define IsUmodez(x)            ((x)->umode & UMODE_z)
#define SetUmodez(x)           ((x)->umode |= UMODE_z)
...
#define ClearUmodeC(x)		((x)->umode &= ~UMODE_C)
#define ClearUmodeH(x)          ((x)->umode &= ~UMODE_H)
#define ClearUmodeW(x)          ((x)->umode &= ~UMODE_W)
...
#define IsDeaf(x)		((x)->umode & UMODE_Y)
#define SetDeaf(x)		((x)->umode |= UMODE_Y)
#define ClearDeaf(x)	((x)->umode &= ~UMODE_Y)
...
#define OFLAG_UMODEC	0x02000000  /* Oper can set umode +C : Global connects */
#define OFLAG_UMODEH	0x04000000  /* Oper can set umode +H : hides their /whois stats */
#define OFLAG_GLINE   0x08000000            /* Oper can gline */
#define OFLAG_UNGLINE 0x20000000         /* Oper can Ungline */
...
#define OFLAG_GLOBAL	(OFLAG_LOCAL|OFLAG_GROUTE|OFLAG_GKILL|OFLAG_GNOTICE|OFLAG_GLINE|OFLAG_UNGLINE)
#define OFLAG_ISGLOBAL	(OFLAG_GROUTE|OFLAG_GKILL|OFLAG_GNOTICE|OFLAG_SADMIN|OFLAG_UMODEF|OFLAG_GLINE|OFLAG_UNGLINE)
...
#define OPCanGline(x)	        ((x)->oflag & OFLAG_GLINE)
#define OPCanUNGline(x)	        ((x)->oflag & OFLAG_UNGLINE)
...
#define OPCanUModeC(x)		((x)->oflag & OFLAG_UMODEC) 
#define OPCanUModeH(x)		((x)->oflag & OFLAG_UMODEH)
#define OPCanUModeF(x)		((x)->oflag & OFLAG_UMODEF)
...
#define OPClearUModeC(x)	((x)->oflag &= ~OFLAG_UMODEC)
#define OPClearUModeH(x)	((x)->oflag &= ~OFLAG_UMODEH)
...
#define FLAGS_RQUIT        0x0100
#define FLAGS_AUTOUMODE_R  0x0200
#define FLAGS_AUTOUMODE_v  0x0400
#define FLAGS_LETUMODE_z   0x0800
#define FLAGS_LETUMODE_v   0x01000
#define FLAGS_LETSMODE_t   0x02000
#define FLAGS_LETSMODE_B   0x04000
...
struct User
{
	...
	int        special;        /* elmer/silly routines */
	char       special_mode;
...
extern char version[128], *solidtext[], *daltext[];
